...
generate_printer_service_files() {
    # Generate Avahi service files for each shared CUPS printer.
    #
    # CUPS's built-in dnssd advertising (BrowseLocalProtocols dnssd) is
    # unreliable — it often fails silently, especially with USB printers
    # using the PWG Raster driver. We write explicit Avahi service files
    # which are 100% reliable for Windows/iOS/Android discovery.
    #
    # Each file advertises _ipp._tcp on port 631 with the TXT records
    # that Windows and AirPrint clients expect.

    # Remove stale service files first (printers that no longer exist)
    rm -f "$AVAHI_SERVICES_DIR"/printer-*.service 2>/dev/null
    # Also remove old AirPrint-* files from previous versions
    ...
    while IFS= read -r printer; do
        ...
        # Skip ghost queues pointing to /dev/null
        local dev_uri
        ...
        # Get printer info for TXT records
        local location description
        location=$(lpoptions -p "$printer" 2>/dev/null | grep -oP 'printer-location=\K[^ ]*' || echo "")
        description=$(lpoptions -p "$printer" 2>/dev/null | grep -oP 'printer-info=\K[^ ]*' || echo "$printer")

        local service_file="$AVAHI_SERVICES_DIR/printer-${printer}.service"
        cat > "$service_file" << SVCEOF
<?xml version="1.0" standalone='no'?>
<!DOCTYPE service-group SYSTEM "avahi-service.dtd">
<!-- Auto-generated by configure-avahi.sh for CUPS printer: $printer -->
<service-group>
  <name replace-wildcards="yes">$printer @ %h</name>

  <!-- IPP (Internet Printing Protocol) for Windows/iOS/Android discovery -->
  <service>
    <type>_ipp._tcp</type>
...
SVCEOF
        ((count++))
        log_info "Created Avahi service file for printer: $printer"
